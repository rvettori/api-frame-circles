      ...
      response(422, 'unprocessable entity - validation errors') do
        ...
        context 'with invalid parameters - missing required fields' do
          ...
          run_test! do |response|
            data = JSON.parse(response.body)
            expect(response.status).to eq(422)
            expect(data['center_x']).to include("can't be blank")
            expect(data['center_y']).to include("can't be blank")
            expect(data['height']).to include("can't be blank")
            expect(data['width']).to include("can't be blank")
          end
        end

        context 'with invalid circles - circle outside frame' do
          let(:frame) do
            {
              frame: {
                center_x: 15.0,
      ...
      response(422, 'unprocessable entity - validation errors') do
        ...
        context 'with missing required fields' do
          ...
          run_test! do |response|
            ...
            expect(data['center_x']).to include("can't be blank")
            expect(data['center_y']).to include("can't be blank")
            expect(data['radius']).to include("can't be blank")
          end
        end
        ...
        context 'with negative radius' do
          let(:frame) { create(:frame) }
          let(:id) { frame.id }
          let(:circle) do
            {
